...
class UtentiDAO extends Connessione//legge db,aggiorna db,ritorna oggetto di itpo uetnte con i dati del db.
{
    ...
    public function findAll()
    {
        ...
        try {
            ...
            $elencoUtenti = parent::getConnessione()->prepare($query);//parent richiama connessione(super)

            $elencoUtenti->execute();//esegue query

            $elencoUtenti->setFetchMode(PDO::FETCH_CLASS, "Utente");
            //fetchclass trasforma righe del record con dati utente

            $Utenti = array();

            while ($utente = $elencoUtenti->fetch()) {
                $Utenti[] = $utente;
            }
            return $Utenti;
